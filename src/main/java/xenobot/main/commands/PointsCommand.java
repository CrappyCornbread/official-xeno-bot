...
public class PointsCommand implements ICommand {
    ...
    public PointsCommand(Database database) {
    ...
    public void handle(List<String> args, GuildMessageReceivedEvent event) {
        ...
        if (target.getUser().isBot()) {
            event.getChannel().sendMessage(event.getAuthor().getAsMention() + ", bots don't have points. We're useless.").queue();
            return;
        }

        int points = database.getPoints(target.getIdLong());
    ...
    private void generateAndSendEmbed(GuildMessageReceivedEvent event, int points, String name) {
        ...
        EmbedBuilder builder = new EmbedBuilder()
                .addField(name, "You currently have **" + points + " points**.", false)
    ...
    public String getHelp() {
        return "Returns how many points a user has.\n\n" +
